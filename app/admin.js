...
const uploadStorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const names = { logo: 'logo.png', favicon: 'favicon.png', bgvideo: 'bg-video.mp4' };
        cb(null, names[req.query.type] || file.originalname);
    }
});
  ...
  if (!req.user || !req.user.email || !req.user.id) return res.redirect('/login/discord');
    if (await db.get(`admin-${req.user.email}`) == true) {
        res.render('admin', {
            ...
            settings: {
                logo: fs.existsSync(path.join(__dirname, '..', 'public', 'assets', 'logo.png')),
                favicon: fs.existsSync(path.join(__dirname, '..', 'public', 'assets', 'favicon.png')),
                bgVideo: fs.existsSync(path.join(__dirname, '..', 'public', 'assets', 'bg-video.mp4'))
            }
        });
    } else {
...
// Upload logo/favicon/bg-video
router.post('/admin/upload', ensureAuthenticated, upload.single('file'), async (req, res) => {
    if (!req.user || !req.user.email) return res.redirect('/login/discord');
    if (await db.get(`admin-${req.user.email}`) != true) return res.redirect('/dashboard');
    res.redirect('/admin?success=UPLOAD');
});

// Remove asset
router.get('/admin/remove-asset', ensureAuthenticated, async (req, res) => {
    if (!req.user || !req.user.email) return res.redirect('/login/discord');
    if (await db.get(`admin-${req.user.email}`) != true) return res.redirect('/dashboard');
    const assetMap = { logo: 'logo.png', favicon: 'favicon.png', bgvideo: 'bg-video.mp4' };
    const file = assetMap[req.query.type];
    if (file) {
        const fp = path.join(__dirname, '..', 'public', 'assets', file);
        if (fs.existsSync(fp)) fs.unlinkSync(fp);
    }
    res.redirect('/admin?success=REMOVED');
});
